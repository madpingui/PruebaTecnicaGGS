...
namespace KartGame.KartSystems
{
    ...
    public class KartAnimation : MonoBehaviour
    {
        ...
        public Transform[] tires1;
        public Transform[] tires2;

        void Start()
        {

            //Animation the wheels depending on the kind of tires, they are facing different directions in the model
            if (PlayerPrefs.HasKey("changeWheels") && tag == "Player")
            {
                if (PlayerPrefs.GetInt("changeWheels") == 0)
                {
                    frontLeftWheel.wheelTransform = tires1[0];
                    frontRightWheel.wheelTransform = tires1[1];
                    rearLeftWheel.wheelTransform = tires1[2];
                    rearRightWheel.wheelTransform = tires1[3];

                    rearLeftWheel.axelAxis = Vector3.right;
                    rearRightWheel.axelAxis = Vector3.right;
                }
                else
                {
                    frontLeftWheel.wheelTransform = tires2[0];
                    frontRightWheel.wheelTransform = tires2[1];
                    rearLeftWheel.wheelTransform = tires2[2];
                    rearRightWheel.wheelTransform = tires2[3];

                    rearLeftWheel.axelAxis = Vector3.forward;
                    rearRightWheel.axelAxis = Vector3.forward;
                }
            }
